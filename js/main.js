...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 7. İletişim Formu Doğrulama ve Formspree Gönderimi
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', async (e) => {
      ...
      if (submitBtnText) submitBtnText.textContent = 'Mesajınız İletiliyor...';
      ...
      try {
        ...
        if (response.ok) {
          contactForm.reset();
          showToast(`Teşekkürler Sayın ${name}! Mesajınız ve teklif talebiniz Can Yıldırım'a ulaştı. En kısa sürede dönüş sağlanacaktır.`, 'success');
        } else {
          const errorData = await response.json().catch(() => null);
          const errorMsg = errorData && errorData.errors ? errorData.errors.map(err => err.message).join(', ') : 'Mesaj iletilirken bir sorun oluştu.';
          showToast(`Hata: ${errorMsg}`, 'warning');
        }
      } catch (error) {
        showToast('Bağlantı hatası oluştu. Lütfen internet bağlantınızı kontrol edip tekrar deneyiniz.', 'warning');
      } finally {
